...
# use networkx.erdos_renyi_graph(N,density) == method b)
...
def get_characteristics(filename,R):
	Random_Gb = get_random_graph_b(filename,R)
	N = nx.number_of_nodes(Random_Gb)		#total number of nodes : N
	L = nx.number_of_edges(Random_Gb)  		#total number of links : L
	Compon = nx.number_connected_components(Random_Gb) #number of connected components
	cc = nx.average_clustering(Random_Gb)	# clustering coefficient : cc
	D = nx.density(Random_Gb)				# network density: Kappa
	check_sum = 0.
	degree_hist = {}
	values = []
	for node in Random_Gb:
		if Random_Gb.degree(node) not in degree_hist:
			degree_hist[Random_Gb.degree(node)] = 1
		else:
			degree_hist[Random_Gb.degree(node)] += 1
		values.append(Random_Gb.degree(node))	
...
def get_number_of_edges_and_density(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_edges_and_density.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		L = nx.number_of_edges(Random_Gb)
		D = nx.density(Random_Gb)
		f.write("%f\t%d\t%f\n" % (R,L,D))
...
def get_average_cluster_coefficient(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_cluster_coeffi_ave.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		cc = nx.average_clustering(Random_Gb)		
		f.write("%f\t%f\n" % (R,cc))
...
def get_degrees_ave(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_degree_ave.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		N = nx.number_of_nodes(Random_Gb) 
		values = []
		for node in Random_Gb:
			values.append(Random_Gb.degree(node))
...
def get_connected_components(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_connected_compo.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
...
def get_shortest_pathway(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_shortest_path.dat','w')
	#f.write('r(thre.)\tshorthest_pathlength\n')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		Compon = nx.connected_component_subgraphs(Random_Gb) # components
		values_2 = []
...
def get_local_efficiency(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_local_efficency_ave.dat','w')
	g = open(input_mtx[:-4]+'_Rb_local_efficency_node.dat','w')
	#g.write('node\tr(thre.)\tlocal_eff')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		local_effic = 0
		for node_i in Random_Gb:
			hiwi = 0.	
			if Random_Gb.degree(node_i)>1:
				neighborhood_i = Random_Gb.neighbors(node_i)
				for node_j in neighborhood_i:
					for node_h in neighborhood_i:
						if node_j != node_h:   #?
							hiwi +=1./nx.shortest_path_length(Random_Gb,node_j,node_h)			
				A = Random_Gb.degree(node_i) * (Random_Gb.degree(node_i) -1.)					
				local_effic +=hiwi / A				
				g.write('%d\t%f\t%f\n' % ( (node_i+1), R, (hiwi/A) ) )
				...
				g.write('%d\t%f\t%f\n' % ((node_i+1), R, hiwi))
		g.write("\n")
		local_effic = local_effic / nx.number_of_nodes(Random_Gb)
		f.write("%f\t%f\n" % ( R, local_effic))
...
def get_global_effic(input_mtx): 
	R = 0
	f = open(input_mtx[:-4]+'_Rb_global_efficiency_ave.dat','w')
	g = open(input_mtx[:-4]+'_Rb_global_efficiency_node.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		global_eff = 0.
		for node_i in Random_Gb:
			sum_inverse_dist = 0.
			for node_j in Random_Gb:
				if node_i != node_j:
					if nx.has_path(Random_Gb, node_i, node_j) == True:
						sum_inverse_dist += 1. / nx.shortest_path_length(Random_Gb, node_i, node_j)
			A = sum_inverse_dist / nx.number_of_nodes(Random_Gb)  # ?
			g.write('%d\t%f\t%f\n' % ((node_i+1), R, A))
			#1.node, 2,threshold, 3.global efficiency of node
			global_eff += sum_inverse_dist / (nx.number_of_nodes(Random_Gb) - 1.) 
		g.write("\n")
		global_eff = global_eff / nx.number_of_nodes(Random_Gb)
		f.write("%f\t%f\n" % (R, global_eff))
...
def get_degree_distribution(input_mtx):			# degree distribution
	R = 0
	f = open(input_mtx[:-4]+'_Rb_degree_dist.dat', 'w')
	...
	for i in range(0,101):
		...
		for node in Random_Gb:
			if Random_Gb.degree(node) not in degree_hist:	
				...
				degree_hist[Random_Gb.degree(node)] += 1
		#degrees = range(0, nx.number_of_nodes(Random_Gb)+1,1)
		degrees = range(1, nx.number_of_nodes(Random_Gb)+1,1)		
		keys = degree_hist.keys()	#keys of block
		keys.sort
		for item in degrees:
			if item in keys:
				P_k=float(degree_hist[item]) / float(nx.number_of_nodes(Random_Gb))
				check_sum +=P_k				
...
def get_node_clustering_coefficient(input_mtx):   # cluster coefficient of each node
	R = 0 
	f = open(input_mtx[:-4]+'_Rb_cluster_coeffi_node.dat','w')			
	#f.write('node\tr(thre.)\tnode_cc\n')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		for node in Random_Gb:
			f.write("%d\t%f\t%f\n" % (node+1, R, nx.clustering(Random_Gb,node)))
...
def get_connected_components_nodes(input_mtx):		# connected components of nodes
	R =0
	f = open(input_mtx[:-4]+'_Rb_connected_compo_node.dat','w')
	#f.write('node\tr(thre.)\tcount\n')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		comps = nx.connected_component_subgraphs(Random_Gb)
		count = 0
		for graph in comps:
			count +=1
			liste = graph.nodes()
...
def get_degrees_node(input_mtx): #degree (links) of each node
	R = 0
	f = open(input_mtx[:-4]+'_Rb_degree_node.dat','w')	
	for i in range(0,101):
		#f.write('node\tr(thre.)\tdegree\n')
		R = float(i)/100
		Random_Gb=get_random_graph_b(input_mtx,R)
		for node in Random_Gb:
			degree = Random_Gb.degree(node)
			f.write('%d\t%f\t%d\n' % ( (node+1), R, degree ) )
...
def get_small_worldness(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_small_worldness.dat','w')
	g = open(input_mtx[:-4]+'_Rb_cc_trans_ER.dat','w')	
	#g.write('r(thre.)\t\cc_A\tcc_ER\ttran_A\ttran_ER\n')	
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		ER_graph = nx.erdos_renyi_graph(nx.number_of_nodes(Random_Gb), nx.density(Random_Gb))
		# erdos-renyi, binomial random graph generator ...(N,D:density)	
		cluster = nx.average_clustering(Random_Gb)   # clustering coef. of whole network
		ER_cluster = nx.average_clustering(ER_graph)	#cc of random graph
		
		transi = nx.transitivity(Random_Gb)
		ER_transi = nx.transitivity(ER_graph)
	
		g.write("%f\t%f\t%f\t%f\t%f\n" % (R,cluster,ER_cluster,transi,ER_transi ))
		
		f.write("%f\t%f\t%f" % (R, cluster, ER_cluster))
		components = nx.connected_component_subgraphs(Random_Gb)
...
def get_motifs(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Rb_motifs.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = get_random_graph_b(input_mtx,R)
		tri_dict = nx.triangles(Random_Gb)   #number of triangles around nodes in Random_Gb
		summe = 0
...
#network = get_random_graph_b(input_name, threshold)
