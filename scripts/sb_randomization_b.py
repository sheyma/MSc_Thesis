...
def measures_random_Gb(matrix):
  R = 0
  f=open(matrix[:-4]+'_Random_Gb_network_measures.dat','w')
...
def shortest_path(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_shortest_path.dat','w')
	#f.write('r(thre.)\tshorthest_pathlength\n')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = random_graph_b(input_mtx,R)
...
def global_effic(input_mtx): 
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_global_efficiency.dat','w')
	g = open(input_mtx[:-4]+'_Random_Gb_node_global_efficiency.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = random_graph_b(input_mtx,R)
		global_eff = 0.
...
def local_effic(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_local_efficency.dat','w')
	g = open(input_mtx[:-4]+'_Random_Gb_node_local_efficency.dat','w')
...
def small_worldness(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_small_worldness.dat','w')
	g = open(input_mtx[:-4]+'_Random_Gb_cc_trans_ER.dat','w')	
...
def degree_dist(input_mtx):			# degree distribution
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_degree_dist.dat', 'w')
...
def node_cc(input_mtx):   # cluster coefficient of each node
	R = 0 
	f = open(input_mtx[:-4]+'_Random_Gb_node_cc.dat','w')			
...
def nodes_of_comp(input_mtx):		# connected components of nodes
	R =0
	f = open(input_mtx[:-4]+'_Random_Gb_nodes_comp_.dat','w')
...
def single_degrees(input_mtx): #degree (links) of each node
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_single_degrees.dat','w')	
	for i in range(0,101):
...
def motifs(input_mtx):
	R = 0
	f = open(input_mtx[:-4]+'_Random_Gb_motifs.dat','w')
	for i in range(0,101):
		R = float(i)/100
		Random_Gb = random_graph_b(input_mtx,R)
