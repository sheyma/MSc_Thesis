...
% A_Random_Ga_network_measures.dat :
%       threshold, L, Density, components, CC_average, check_sum, sum_ave

Net_mes = load('A_Random_Ga_network_measures.dat');
Shor_pat = load('A_Random_Ga_shortest_path.dat');
Glo_ef = load('A_Random_Ga_global_efficiency.dat');
Loc_ef = load('A_Random_Ga_local_efficency.dat');
SWorld = load('A_Random_Ga_small_worldness.dat');

% network measures as distributions around nodes
deg_dis = load('A_Random_Ga_degree_dist.dat');
node_CC = load('A_Random_Ga_node_cc.dat');
comp_dist =load('A_Random_Ga_nodes_comp_.dat');
single_de =load('A_Random_Ga_single_degrees.dat');
