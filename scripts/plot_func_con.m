...
legend('R_{BG}', 'R_{ER}','R_{DES}','R_{CM}','R_{PDD}','R_{PR}','Location','SouthWest' )
%%set(legend,'FontSize',14)
legend('boxoff')
xlabel('r')
ylabel('S')
...
legend('R_{BG}', 'R_{ER}','R_{DES}','R_{CM}','R_{PDD}','R_{PR}','Location','south','Orientation','horizontal')
%%set(legend,'FontSize',14)
legend('boxoff')
xlabel('r')
ylabel('A')
...
for j = 1:length(random_G)
    ...
    elseif random_G(j)=='d'
        ...
        linear_plot = imagesc((1:1:90),(0.25:0.01:1),c);
        colormap(jet(1024)), caxis(my_clim)
        cbar = colorbar('peer', linear_axes, 'Yscale', 'log' ,'fontsize',10);

        set(gca,'YTick',[0.25 0.45 0.62 0.80 1.00])
        set(gca, 'YTickLabel', num2str(get(gca,'YTick')','%.2f'))     
    ...
    elseif random_G(j) == 'k'
        ...
        linear_plot = imagesc((1:1:90),(0.25:0.01:0.94),c);
        colormap(jet(1024)), caxis(my_clim)
        cbar = colorbar('peer', linear_axes, 'Yscale', 'log' ,'fontsize',10);

        set(gca,'YTick',[0.25 0.40 0.55 0.75 0.94])
        set(gca, 'YTickLabel', num2str(get(gca,'YTick')','%.2f'))
  
    end
    
   if random_G(j) == 'h' || random_G(j) == 'k'
            xlabel('Nodes', 'fontsize', 30)
   end
